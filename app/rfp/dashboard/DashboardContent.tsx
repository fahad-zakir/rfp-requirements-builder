      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="rounded-2xl border border-[var(--border)] bg-white p-8 shadow-sm">
          ...
          {data.requirements && data.requirements.length > 0 && (
            <div className="mt-10">
              <h2 className="text-lg font-semibold text-[var(--foreground)]">AI-Generated Requirements</h2>
              <p className="mt-1 text-sm text-[var(--text-muted)]">
                Complete list of all {data.requirements.length} requirements generated by AI based on your organization profile and category weightings. These are included in your PDF export.
              </p>
              <div className="mt-4 max-h-96 space-y-3 overflow-y-auto rounded-lg border border-[var(--border)] bg-[var(--background-soft)] p-4">
                {data.requirements.map((req, idx) => {
                  ...
                  const priorityColor = priority ? priorityColors[priority] || "bg-gray-100 text-gray-700 border-gray-200" : "";
                  
                  return (
                    <div key={req.id || idx} className="rounded-lg border border-[var(--border)] bg-white p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            <span className="inline-flex items-center rounded-md bg-[var(--primary-muted)] px-2 py-0.5 text-xs font-medium text-[var(--primary)]">
                              {req.category}
                            </span>
                            {priority && (
                              <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium ${priorityColor}`}>
                                {priority.toUpperCase()}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-[var(--foreground)] leading-relaxed">{req.text}</p>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <p className="mt-3 text-xs text-[var(--text-muted)] italic">
                All {data.requirements.length} requirements are included in your PDF download. This is your complete AI-generated requirements list.
              </p>
            </div>
          )}
